...
mod terminal_state;
use terminal_state::{CELL_SIZE, TerminalState};
...
/// Verify content-addressed resources before they enter browser-global caches.
/// Web Crypto does not expose BLAKE3, while the server uses it for font and
/// extension identities, so keep one small implementation in the existing
/// browser WASM module.
#[wasm_bindgen]
...
export function yasFillTextCodePoint(ctx, codePoint, x, y) {
  ctx.fillText(codePointText(codePoint), x, y);
}

export function yasFillTextStretched(ctx, codePoint, x, y, targetWidth) {
...
export function yasFillText(ctx, text, x, y) {
  ctx.fillText(text, x, y);
}

const PROBE_CHARS = [0x4D, 0x6D, 0x57, 0x77, 0x40, 0x25, 0x23, 0x47, 0x4F, 0x51];

export function yasMeasureMaxOverhang(ctx, cellWidth) {
  let maxOverhang = 0;
...
"#)]
unsafe extern "C" {
    fn yasFillTextCodePoint(ctx: &CanvasRenderingContext2d, code_point: u32, x: f64, y: f64);
    fn yasFillTextStretched(
    ...
    fn yasFillText(ctx: &CanvasRenderingContext2d, text: &str, x: f64, y: f64);
    fn yasMeasureMaxOverhang(ctx: &CanvasRenderingContext2d, cell_width: f64) -> f64;
}
...
impl GlyphAtlas {
    ...
    fn ensure_metrics(&mut self, cell_width: f64, cell_height: f64, font: &str) -> bool {
            ...
            || self.wide_cell_width != wide_width
        {
            ...
            if let Some(ctx) = &self.ctx {
                ctx.set_font(font);
                let overhang = yasMeasureMaxOverhang(ctx, cell_width);
    ...
    fn ensure_glyph(
        ...
        if (0x2500..=0x259F).contains(&code_point) {
            yasFillTextStretched(ctx, code_point, draw_x, draw_y, cell_width);
        } else {
            yasFillTextCodePoint(ctx, code_point, draw_x, draw_y);
        }
        if key.underline {
