...
/// Every yas child becomes a `setsid()` session leader (see `spawn_pty`), so
/// its pgid equals its pid. There is a short post-fork window before that call
/// where `kill(-pid)` returns `ESRCH`; fall back to the leader pid so an
/// immediate signal is not lost. That reaches the leader's own group; a shell
/// puts each job in a *separate* group, so the foreground job is signalled
/// through `TIOCGPGRP` the same way `resize_pty_os` delivers `SIGWINCH`.
/// Backgrounded jobs in neither group still survive — bounding those needs a
/// cgroup, not a signal.
///
...
pub fn kill_pty(handle: &PtyHandle, signal: i32, group: bool) {
    unsafe {
        ...
        if libc::kill(-handle.child_pid, signal) != 0 {
            libc::kill(handle.child_pid, signal);
        }
    }
}
...
mod tests {
    ...
    /// under test still finds a zombie to consume. A concurrently running
    /// supervisor may win the reap and park its status first; `ECHILD` is the
    /// equivalent completed state for these tests.
    fn wait_until_zombie(pid: libc::pid_t) {
        ...
        assert!(
            ret == 0 || std::io::Error::last_os_error().raw_os_error() == Some(libc::ECHILD),
    ...
    /// The parent can receive a terminal signal request before the forked
    /// child has reached `setsid()`. Its future process group does not exist
    /// yet, so the group send must fall back to the leader itself.
    #[test]
    fn group_kill_reaches_the_leader_before_setsid() {
        let pid = super::fork_child();
        assert!(pid >= 0, "fork failed");
        if pid == 0 {
            unsafe {
