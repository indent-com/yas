...
impl Fixture {
    ...
    fn key(&mut self, keycode: u32, pressed: bool) {
        self.key_with_caps(keycode, pressed, None);
    }

    fn key_with_caps(&mut self, keycode: u32, pressed: bool, caps_lock: Option<bool>) {
    ...
    fn text(&mut self, text: &str) {
        self.command(CompositorCommand::TextInput { text: text.into() });
        self.settle();
    }
...
#[test]
fn committed_text_preserves_case_and_restores_caps_and_shift() {
    for caps in [false, true] {
        for shift in [false, true] {
            ...
            let mut depressed = if shift { MOD_SHIFT } else { 0 };
            let mut locked = if caps { MOD_LOCK } else { 0 };
            let mut characters = Vec::new();
            for event in fx.take_log() {
                match event {
                    Ev::Mods(d, l) => (depressed, locked) = (d, l),
                    Ev::Key(key, true) if key == 30 || key == 2 => {
                        let shifted = depressed & MOD_SHIFT != 0;
                        characters.push(if key == 30 {
                            if shifted ^ (locked & MOD_LOCK != 0) {
                                'A'
                            } else {
                                'a'
                            }
                    ...
                    Ev::Key(KEY_K, true) => {
                        assert_eq!(depressed, if shift { MOD_SHIFT } else { 0 });
                        assert_eq!(locked, if caps { MOD_LOCK } else { 0 });
                    }
                    _ => {}
                }
            }
            assert_eq!(
                ...
                "aA1!",
                "committed text with caps={caps}, shift={shift}"
            );
        }
    }
}
...
#[test]
fn a_key_snapshot_recovers_caps_lock_before_the_key() {
    let mut fx = Fixture::new();
    fx.focus(fx.surface_id);
    fx.take_log();

    // A new viewer can arrive with either state; the previous viewer's latch
    // must not determine how this one's first key is interpreted.
    for locked in [true, false, true] {
        fx.key_with_caps(KEY_K, true, Some(locked));
        fx.key_with_caps(KEY_K, false, Some(locked));
        assert_eq!(
            fx.take_log(),
            vec![
                Ev::Mods(0, if locked { MOD_LOCK } else { 0 }),
                Ev::Key(KEY_K, true),
                Ev::Key(KEY_K, false),
            ]
        );
...
#[test]
fn caps_lock_snapshots_are_not_toggled_again_by_presses_or_repeats() {
    let mut fx = Fixture::new();
    fx.focus(fx.surface_id);
    fx.take_log();

    for locked in [true, false] {
        fx.key_with_caps(KEY_CAPSLOCK, true, Some(locked));
        fx.key_with_caps(KEY_CAPSLOCK, true, Some(locked));
        fx.key_with_caps(KEY_CAPSLOCK, false, Some(locked));
        fx.key_with_caps(KEY_K, true, Some(locked));
        fx.key_with_caps(KEY_K, false, Some(locked));
        assert_eq!(
            fx.take_log(),
            vec![
                Ev::Mods(0, if locked { MOD_LOCK } else { 0 }),
                Ev::Key(KEY_CAPSLOCK, true),
                Ev::Key(KEY_CAPSLOCK, true),
                Ev::Key(KEY_CAPSLOCK, false),
                Ev::Key(KEY_K, true),
                Ev::Key(KEY_K, false),
            ]
        );
