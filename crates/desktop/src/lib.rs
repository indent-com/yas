...
fn normalize_menu_layout(root: DBusMenuLayout) -> Option<NormalizedMenu> {
    ...
    // Keep the DBusMenu root in the normalized tree.  The native wire
    // projection assigns handles parent-first and uses the first node as its
    // non-actionable root; dropping it here made the first real entry become
    // that root and left every other top-level entry with an unknown parent.
...
mod dbus_tests {
    ...
    impl TestBus {
        fn spawn() -> (Self, String) {
            let mut command = ProcessCommand::new("dbus-daemon");
            if let Some(config) = std::env::var_os("YAS_TEST_DBUS_SESSION_CONF") {
    ...
    async fn watcher_registers_and_normalizes_a_kde_item() {
        ...
        assert_eq!(menu.nodes[0].id, 0);
        assert_eq!(menu.nodes[0].parent_id, 0);
        assert_eq!(menu.nodes[0].label, "");
        assert_eq!(menu.nodes[1].label, "Keep _open");
        assert_ne!(menu.nodes[1].flags & MENU_NODE_CHECKMARK, 0);
        assert_eq!(menu.nodes[1].toggle_state, 1);
        assert_ne!(menu.nodes[2].flags & MENU_NODE_SUBMENU, 0);
        assert_eq!(menu.nodes[3].parent_id, 3);
