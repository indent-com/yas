...
pub enum Command {
    ...
    /// Interactively manage Muster stacks, units, and terminals
    Mustard,
    ...
    ///
    ///   YAS_WEBTRANSPORT  Set to 1 for a native QUIC datagram listener
    ///
    ///   YAS_WEBTRANSPORT_ADDR  UDP bind (defaults to YAS_ADDR)
    ///
...
mod tests {
    ...
    #[test]
    fn extension_manage_replaces_install() {
        let cli = Cli::try_parse_from([
            "yas",
            "ext",
            "manage",
            "--from",
            "http://localhost:10003/ext",
        ])
        .unwrap();
        let Command::Extension {
            command: ExtensionCommand::Manage(args),
        } = cli.command
        else {
            panic!("manage must open the extension manager");
        };
